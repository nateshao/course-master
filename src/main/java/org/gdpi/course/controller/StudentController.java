...
public class StudentController {
     ...
     */
    @GetMapping("/course")
    public ModelAndView initCourse(@AuthenticationPrincipal UserDetails userDetails, ModelAndView mv,
        ...
        List<Course> personalizedCourseList = null;
        if(course.size() == 0){
            personalizedCourseList = studentService.personalizedAllCourse();
        } else {
            personalizedCourseList = studentService.personalizedCourse(stu.getId());
        }

        PageInfo<Course> pageInfo = new PageInfo<Course>(personalizedCourseList);
    ...
    public String courseMain(@AuthenticationPrincipal UserDetails userDetails,
    ...
    public ModelAndView initResources(@AuthenticationPrincipal UserDetails userDetails, ModelAndView mv,@RequestParam(defaultValue = "1") Integer page) {
        ...
        List<Course> courses = studentService.findCourseById(student.getId());
    ...
    public ModelAndView addInvitation(ModelAndView mv, @AuthenticationPrincipal UserDetails userDetails) {
        ...
        Student stu = studentService.findByUsername(userDetails.getUsername());
        List<Course> courses = studentService.findCourseById(stu.getId());
        mv.addObject("courses", courses);
        mv.addObject("user", stu);
